...
'msgbox 5555
  ...
  iStart = LenB  (sSpace)
  iFormStart = iStart+2
'oUpFileStream.savetofile "c:\u.txt"
 'response.write len(RequestBinData ):response.end
  '分解项目
 ' dim k 
  Do
    ...
    '   msgbox iFindStart,iFindEnd
   ' response.write  iFindStart & "->" & iFindEnd:response.end
    else
    '如果是表单项目
      tStream.Close
      tStream.Type = 1
   ...
   ' response.write iFormStart :response.end
    '如果到文件尾了就退出
