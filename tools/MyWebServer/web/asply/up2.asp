...
attachdir=ServerPath&"upload"  '上传文件保存路径，结尾不要带/
dirtype=2'1:按天存入目录 2:按月存入目录 3:按扩展名存目录  建议使用按天存
maxattachsize=2097152*2'最大上传大小，默认是2M
...
Private Sub Class_Initialize
	isErr_ = 0
	NoAllowExt="asp;aspx;php;jsp;cgi"		'黑名单,可以在这里预设不可上传的文件类型,以文件的后缀名来判断,不分大小写,每个每缀名用;号分开,如果黑名单为空,则判断白名单
